...
import extractivesummary as extract
...
def main():
  ...
  # Generate an ordered csv file with text (col0) to label (col1)
  top_n = 3  # Set this to whatever number of sentences you want in the summary
  stop_words = stopwords.words('english')
  df.loc[:, 'claims'] = df['doc-id'].apply(lambda x: generate_summary(claims_dict.get(x, ""), top_n,stop_words) if claims_dict.get(x) is not None else np.nan)
