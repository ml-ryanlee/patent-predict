...
@author: sawsaxen 
@author2: ryanlee(modified for patent examination)
"""
...
def read_article(text):
    ...
    for sentence in sentences:
        ...
        sentence = re.sub(r"\(canceled\s*\)", " ", sentence, flags=re.IGNORECASE)
        sentence = re.sub(r"\(cancelled\s*\)", " ", sentence, flags=re.IGNORECASE)
    return sentences
...
def generate_summary(text,top_n,stop_words):
    ...
    for i in range(top_n):
        # if there are less than 3 sentences in a claims text, don't add a third
